...
class Employee  extends Table{
    ...
    /* Display all the managers from the employee table*/
      ...
      return  mysql.createConnection(this.connection)
                 .then(conn=>{
                    ...
                    `SELECT first_name,last_name from 
                      ${this.name} E where id IN (select distinct manager_id from ${this.name})  
                    `)
    ...
    /*Display the employee associated to a manager id */
     viewEmployeesByManagerName(managerName){

       return  mysql.createConnection(this.connection)
                 .then(conn=>{
                  return  conn.query(`select first_name,last_name from ${this.name} E where manager_id IN 
                    (SELECT  M.id  from ${this.name} M WHERE M.first_name="${managerName.split('\t')[0]}" AND M.last_name="${managerName.split('\t')[1]}")`)
    ...
    add(data){
        ...
        return  mysql.createConnection(this.connection)
                 .then(conn=>{
                    conn.query(`INSERT INTO ${this.name}(${this.columns.slice(1).join(",")}) VALUES("${first_name}","${last_name}",(SELECT id from role where title="${role_name}"),(SELECT id from employee as E where E.first_name="${manager_name.split("\t")[0]}" AND E.last_name="${manager_name.split("\t")[1]}"))`)
                 })
                 .catch(error=>console.log(error));
